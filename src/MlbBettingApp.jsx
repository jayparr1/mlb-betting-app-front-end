  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:justify-center gap-4 mb-6">
        <button
          className={showEVOnly ? "px-4 py-2 rounded-full border bg-green-500 text-white" : "px-4 py-2 rounded-full border bg-white"}
          onClick={() => setShowEVOnly(!showEVOnly)}
        >
          {showEVOnly ? "Showing +EV Only" : "Show +EV Only"}
        </button>
        <button
          className={showParlayOnly ? "px-4 py-2 rounded-full border bg-blue-500 text-white" : "px-4 py-2 rounded-full border bg-white"}
      ...
      {loading ? (
        ...
        <p className="text-center text-red-600">No valid picks returned from server.</p>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredPicks.map((pick, index) => (
            <div key={index} className="border rounded-2xl bg-white shadow-md p-6 text-sm md:text-base space-y-2">
